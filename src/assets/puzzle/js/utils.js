...
function Utility() {
  ...
  // What is the isDoubleTapBug? Some Galaxy devices pass out 2 events for every single tap. Yes. Google for it.
  isDoubleTapBug: function(evt) {
    if (!('ontouchstart' in document.documentElement) || !$.browser.android) return false;
  ...
  setColorScheme: function(c1, c2) {
    ...
    var css = '' +
      '.odd  .tile-1 .inner { background-color: ' + c1 + ';background-repeat: no-repeat;background-size: 80%;background-image:url("/assets/puzzle//img/coconut.png");background-position:center center;color: #000 }' +
      '.even .tile-1 .inner { background-color: ' + c1 + ';background-repeat: no-repeat;background-size: 80%;background-image:url("/assets/puzzle//img/coconut.png");background-position:center center;color: #000 }' +
      '.odd  .tile-2 .inner { background-color: ' + c2 + '; }' +
      '.even .tile-2 .inner { background-color: ' + c2 + '; }';
    Utils.createCSS(css);
  }
...
$.browser.chromeWebStore = (window.chrome && window.chrome.storage)? true : false;

for (var o in $.browser)
  if ($.browser[o]) $('html').addClass(o);